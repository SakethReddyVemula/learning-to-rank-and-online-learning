...
class FeatureExtractor:
    def __init__(self):
        # User profiles: user_id -> {topic: count}
        self.user_profiles = defaultdict(lambda: defaultdict(int))
        # Article cache: article_id -> article_data
        self.article_cache = {}
    ...
    def get_features(self, user_id, article_data, query_text):
        ...
        # Interaction features (User-Topic Affinity)
        topic_match_score = 0
        if total_clicks > 0:
        ...
        # Simple text match features (could be expanded)
